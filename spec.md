...
- **Piece 1**: the tested screen with its interaction heatmap — from a Nova journey run
  ...
  *Headless verified against the live Space: all AI endpoints are csrf-exempt POST-JSON —
  `/health/` answers 200 and `/generate_heatmap/` accepts unauthenticated calls, failing only on
  the missing `GLOBAL_SEARCH_API_KEY` (Gemini) Space secret. **No full-code integration needed**;
  set that secret (plus `OPENAI_API_KEY_SULTAN` fallback) on the Space. Contracts (from source):
  heatmap takes `{figma_data: <Figma file JSON>, user_prompt}` and returns per-node
  `{heatmap, report, suggestions, positive_points, drop_off_points, ux_score}`; iteration takes
  `{figma_token, design_data: {id}, dropoff_points}` and returns the improved design as base64.
  ux-mentor is Figma-data-driven — screenshot-only chains use the journey-run heatmap instead.*
