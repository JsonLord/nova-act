...
abi/screenshot-to-code fork), `NOVA_ACT_API_KEY`, `USERSYNC_ENGINE` (open|nova|auto), `USERSYNC_PERCEPTION` (cpu|zerogpu|auto — the CPU/ZeroGPU toggle), `OMNIPARSER_BASE_URL`, `USERSYNC_PUBLIC_BASE_URL`, `USERSYNC_MAX_WORKERS` (bounded job/browser concurrency, default 2), `USERSYNC_STORAGE` (sqlite [default, WAL DB] | files), `NEO4J_URI` / `NEO4J_USER` / `NEO4J_PASSWORD` / `NEO4J_DATABASE` (graph store — HF Secrets; placeholder connection, status at `/api/graph-store/status`), `FREE_CREDITS`. LLM access is strictly BYOK — no server-side model keys.
